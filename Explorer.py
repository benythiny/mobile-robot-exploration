...
class Explorer:
    ...
    def mapping(self):
        ...
        """
        print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ", "Started the mapping thread") 
        while not self.stop:
            ...
            grid_map = self.explor.fuse_laser_scan(grid_map, laser_scan, odometry)
            ...
            gridmap_with_other_robot = copy.deepcopy(grid_map)
    ...
    def goal_is_still_a_frontier(self):
        ...
        if frontier_still_there == False:
            print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ", "The old frontier goal is not there anymore. Rerouting to the next closest frontier...")
            self.path = None
            self.robot.navigation_goal = None
    ...
    def planning(self):
        ...
        print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Planning thread started.")
        ...
        while not self.stop:
            ...
            if self.planning_method == 3:
                ...
                if len(current_frontiers) == 1:
                    ...
                    print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Started planning the path with TSP.")
                    ...
                    if len(TSP_result) < 2:
                        ...
                        print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","TSP result was ", TSP_result)
                        continue
                ...
                if current_path is not None:
                    ...
                    print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Path was successfully planned.")
                    print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Going to frontier at x: ", "{:.2f}".format(self.goal_frontier.position.x), ", y:", "{:.2f}".format(self.goal_frontier.position.y))
                    
                    self.terminate_counts = 10
                else:
                    print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","TSP solution was not found, couldn't plan path.")
                    print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Counts before termination: ", self.terminate_counts)
                    self.frontiers = []
                    self.terminate_counts -= 1
                ...
                print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Started planning the path.")
                ...
                for f in self.frontiers:
                    ...
                    if self.path is not None:       
                        ...
                        print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Path was successfully planned.")
                        print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","Going to frontier at x:", "{:.2f}".format(self.goal_frontier.position.x), 
                        ...
                        print(time.strftime("%H:%M:%S"), "| id:", self.id, "| ","No path was found.")
